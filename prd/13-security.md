...
#### 登记例外 #2：IP 归属地查询（v0.1.2 新增）

「IP 地址解析器」工具的归属地/运营商查询（Country/Region/City/ISP/ASN/时区）需要联网获取第三方数据。作为**用户手动触发的受控例外**，实现约束：

- 仅使用 `src-tauri/src/net/ip_lookup.rs` 中硬编码白名单域名：`ip-api.com`（数据）、`flagcdn.com`（国旗 PNG）
- 查询目标 IP 先经过字符白名单校验（仅 `[0-9a-fA-F.:]`），杜绝 URL 路径注入
- **仅在用户点击「查询归属地」按钮时发起**，不自动请求、无遥测、不持久化查询记录
- 网络依赖为 `ureq`（rustls + gzip + json，最小特性集）；CSP `connect-src` 不放宽，前端仍禁止直连外网，全部经 Rust IPC 代理
