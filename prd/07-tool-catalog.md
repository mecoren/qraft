...
Qraft 计划内置 34 个开发工具（P0=10 / P1=12 / P2=12），但一次性全部实现不现实。本文档的目标是：
...
10 个 P0 工具覆盖了六大分类中的五个（Comparator 除外，Diff 类工具推迟到 v1.0），并验证了全部架构特性：
...
> 📌 **分类说明**
>
> `hash_calculator` / `hash_text` / `hmac_generator` 归入 `Encoder` 类，此处"Encoder"泛指编解码与单向转换类工具（含哈希/摘要计算）。若未来哈希类工具增多，可独立为 `Crypto/Hashing` 分类。
...
Qraft 计划在 v2.0 达到 34 个工具。若超过 50 个，需要：
