 ...
 *
 * 目标行为(App.tsx 的 openFileInMarkdownEditor 依赖):
...
import type { MdDocsWorkspace } from './markdownEditorDocsStore';
...
function resetStore(
  partial: Partial<Parameters<typeof useMdEditorDocsStore.setState>[0]> = {},
): void {
...
describe('openDocFromSystem', () => {
  ...
  it('追加新文档并激活,不影响既有文档', () => {
    useMdEditorDocsStore.getState().openDocFromSystem('# 注入文档\n\n内容');
  ...
  it('hydrate 未完成时调用:不置位 userTouched,后续 hydrate 合并保住持久化文档', async () => {
    useMdEditorDocsStore.getState().openDocFromSystem('# 系统打开');
    expect(useMdEditorDocsStore.getState().userTouched).toBe(false);
    ...
    await useMdEditorDocsStore.getState().hydrate();
  ...
  it('ready 后调用:置位 userTouched,防抖 persist 可落盘', () => {
    ...
    useMdEditorDocsStore.getState().openDocFromSystem('# 运行中打开');
    expect(useMdEditorDocsStore.getState().userTouched).toBe(true);
  });

  it('首次使用(hydrate 前注入,无持久化数据):注入文档保留,不被示例文档/firstUse 覆盖', async () => {
    useMdEditorDocsStore.getState().openDocFromSystem('# 我刚打开的文档');
    expect(useMdEditorDocsStore.getState().userTouched).toBe(false);
  ...
  it('首次使用且注入空 .md(空内容):文档仍保留,不退回示例文档', async () => {
    useMdEditorDocsStore.getState().openDocFromSystem('');
    invokeMock.mockResolvedValue({ ok: true, value: null });
  ...
  it('内容无标题行时以首个非空行做标题;内容太短则回退 md-N 自动命名', () => {
    ...
    useMdEditorDocsStore.getState().openDocFromSystem('只是普通文本');
    const doc = useMdEditorDocsStore.getState().docs.find((d) => d.content === '只是普通文本');
    expect(doc?.title).toBe('只是普通文本');
    expect(doc?.autoTitle).toBeUndefined();
    // 内容过短(≤3 字符)不派生标题:回退 md-N 自动命名
    useMdEditorDocsStore.getState().openDocFromSystem('ab');
    const fallback = useMdEditorDocsStore.getState().docs.find((d) => d.content === 'ab');
  ...
  it('携带文件载荷(拖放/关联打开):走绑定语义,同路径去重刷新而非重复 Tab', () => {
    ...
    const first = useMdEditorDocsStore.getState().docs.find((d) => d.path === 'C:\\docs\\drop.md');
    ...
    expect(useMdEditorDocsStore.getState().activeDocId).toBe(first?.id);
    ...
    const after = useMdEditorDocsStore.getState();
  ...
  it('无载荷(纯内容注入)保持既有非绑定语义:不设 path/savedContent', () => {
    useMdEditorDocsStore.getState().openDocFromSystem('# 纯内容');
    const doc = useMdEditorDocsStore.getState().docs.find((d) => d.content === '# 纯内容');
...
describe('文件绑定(openFileAsDoc / attachPath / markSaved)', () => {
  ...
  it('openFileAsDoc:同路径文档已存在时重读刷新内容与基准,不追加新 Tab', () => {
    ...
    useMdEditorDocsStore
      .getState()
      .setDocContent(useMdEditorDocsStore.getState().activeDocId!, 'v1 改');
  ...
  it('attachPath:纯草稿另存为后绑定路径,标题切为文件名且内容派生让位', () => {
    ...
    useMdEditorDocsStore.getState().setDocContent(id, '# 草稿标题 改');
    expect(useMdEditorDocsStore.getState().docs.find((d) => d.id === id)?.title).toBe('saved.md');
  });
  ...
  it('markSaved:刷新 savedContent 快照与 mtime;dirty 判定随内容变化', () => {
    ...
    const id = useMdEditorDocsStore.getState().activeDocId!;
    // 打开后干净
    let doc = useMdEditorDocsStore.getState().docs.find((d) => d.id === id);
  ...
  it('持久化往返:path/encoding/mtimeMs/savedContent 一并还原(sanitizeDoc)', async () => {
    ...
    await useMdEditorDocsStore.getState().persistDocs();
  ...
  it('hydrate 还原带 path 的文档:字段完整还原', async () => {
    ...
    useMdEditorDocsStore.setState({ ready: false, userTouched: false });
    await useMdEditorDocsStore.getState().hydrate();
    const doc = useMdEditorDocsStore.getState().docs[0];
