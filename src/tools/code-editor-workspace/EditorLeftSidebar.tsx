...
export function EditorLeftSidebar({
  ...
  return (
    ...
    >
      ...
      {/* 列表:文件列表 + 对比差异分组,共用一个滚动容器(对齐 VSCode EXPLORER 多分组) */}
      {!collapsed && (
        <ScrollArea className="min-h-0 flex-1">
          ...
          {/* 对比差异 分组:每次「比较所选内容」在此新增一条,点击后主区域直接显示 diff */}
          {compares.length > 0 && (
            <section aria-label="对比差异" className="mt-1">
              <div
                ...
                className="flex min-w-0 cursor-pointer select-none items-center gap-1 border-b border-sidebar-border px-3 py-2 text-[10px] font-semibold uppercase tracking-wider text-muted-foreground hover:bg-sidebar-accent/40 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
              >
                <ChevronDown
                  aria-hidden
                  className={cn(
