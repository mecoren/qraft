...
const ROWS: ReadonlyArray<{ key: keyof TextStats; labelKey: string }> = [
  { key: 'chars', labelKey: 'tools.text_statistics.chars' },
  { key: 'charsNoSpaces', labelKey: 'tools.text_statistics.chars_no_spaces' },
  { key: 'words', labelKey: 'tools.text_statistics.words' },
  { key: 'lines', labelKey: 'tools.text_statistics.lines' },
  { key: 'bytes', labelKey: 'tools.text_statistics.bytes' },
];
...
export function TextStatistics({ toolId }: ToolProps): JSX.Element {
  ...
  const summary = ROWS.map((r) => `${t(r.labelKey)}: ${stats[r.key]}`).join('\n');
    ...
    >
      ...
      <ResizablePanelGroup orientation="horizontal" className="min-h-0 flex-1">
        <ResizablePanel defaultSize={55} minSize={20}>
          <CodeEditor
            title={t('tools.text_statistics.input_title')}
            value={text}
          ...
          >
            <div className="flex items-center justify-between border-b px-3 py-1.5">
              <span className="flex-1 text-xs font-medium text-muted-foreground">
                {t('tools.text_statistics.stats_title')}
              </span>
            ...
            <div className="flex-1 overflow-auto p-3">
              <dl className="grid gap-2">
                {ROWS.map((r) => (
                  ...
                  >
                    <dt className="text-xs text-muted-foreground">{t(r.labelKey)}</dt>
